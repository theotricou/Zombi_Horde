...
class Zombi():
    ...
    def T(self, parameters_file, experiment_folder, advanced_mode, rep_seed):
        ...
        if advanced_mode == "i":
            ...
            stg = SpeciesTreeGenerator(parameters, rep_seed)

        os.system("cp " + parameters_file + " " + tree_folder)
        os.system("sed -i 's/SEED 0/SEED " + str(rep_seed) + "/g' " + tree_folder + "/" + parameters_file.split('/')[-1])

        run_counter = 0
        success = False

        while success == False and run_counter < 1000:
            run_counter += 1
    ...
    def G(self, parameters_file, experiment_folder, advanced_mode, rep_seed):
        ...
        os.system("sed -i 's/SEED 0/SEED " + str(rep_seed) + "/g' " + genome_folder + "/" + parameters_file.split('/')[-1])

        genomes_folder = os.path.join(genome_folder, "Genomes")
        gene_families_folder = os.path.join(genome_folder, "Gene_families")

        gss = GenomeSimulator(parameters, events_file, rep_seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("rep", type=int, help="Number of Species tree replicate")
    parser.add_argument("seed", const=0, default=0 ,nargs='?', type=int, help="Simulations are reproductible for a given set of parameters and a specific seed")
    parser.add_argument("threads", type=int, default=1, const = 1, nargs='?', help="number of thread for paralle job")

    args = parser.parse_args()

    mode, parameters_file, pre_experiment_folder = args.mode, args.params, args.output
    ...
    def my_function(X):
        ...
        experiment_folder = str(pre_experiment_folder + str(X[1]))
        rep_seed = X[0]
        Z = Zombi()
        ...
        if main_mode == "T":
            ...
            if not os.path.isdir(os.path.join(experiment_folder, "T")):
                ...
                Z.T(parameters_file, experiment_folder, advanced_mode, rep_seed)
            ...
            if not os.path.isdir(genome_folder):
                ...
                Z.G(parameters_file, experiment_folder, advanced_mode, rep_seed)
            ...
            if not os.path.isdir(sequences_folder):
                os.mkdir(sequences_folder)
                Z.S(parameters_file, experiment_folder, advanced_mode, rep_seed)
            else:
